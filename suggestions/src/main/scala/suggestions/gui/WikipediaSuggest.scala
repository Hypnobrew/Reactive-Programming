...
object WikipediaSuggest extends SimpleSwingApplication with ConcreteSwingApi with ConcreteWikipediaApi {
  ...
  def top = new MainFrame {
    ...
    val suggestionSubscription: Subscription =  suggestions.observeOn(eventScheduler) subscribe {
      x => x match {
        case Success(suggestion) => suggestionList.listData = suggestion
        case Failure(error) => status.text = error.getLocalizedMessage()
      }
    }
    ...
    // TO IMPLEMENT
    val pages: Observable[Try[String]] = selections.concatRecovered(p => wikiPageResponseStream(p))

    // TO IMPLEMENT
    val pageSubscription: Subscription = pages.observeOn(eventScheduler) subscribe {
      x => x match {
        case Success(page) => editorpane.text = page
        case Failure(error) => error.getLocalizedMessage()
      }
    }
  }

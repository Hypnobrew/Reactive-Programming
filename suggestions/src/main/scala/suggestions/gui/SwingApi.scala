...
trait SwingApi {
  ...
  implicit class TextFieldOps(field: TextField) {
    ...
    def textValues: Observable[String] = Observable.create((observer: Observer[String]) => {
      field subscribe { case ValueChanged(textField) => observer.onNext(textField.text)}
      Subscription {}
    })
  }
  ...
  implicit class ButtonOps(button: Button) {
     ...
     */
    def clicks: Observable[Button] = Observable.create((observer: Observer[Button]) => {
      button subscribe { case ButtonClicked(button) => observer.onNext(button)}
      Subscription {}
    })
  }
}
